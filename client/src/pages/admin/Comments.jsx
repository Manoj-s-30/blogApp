...
import CommentsTableItem from '../../components/CommentsTableItem'
...
const Comments = () => {
    ...
    const fetchComments = async () => {
        try {
            const { data } = await axios.get('http://localhost:3000/api/admin/comments');
            ...
            <div className='relative h-4/5 max-w-3xl overflow-x-auto mt-4 bg-white shadow rounded-lg scrollbar-hide'>
                <table className='w-full text-sm text-gray-500'>
                    ...
                    <tbody>
                        ...
                        }).map((comment, index) => <CommentsTableItem key={comment._id} index={index + 1} fetchComments={fetchComments} comment={comment} />)}
                    </tbody>
                </table>
            </div>
        </div>
